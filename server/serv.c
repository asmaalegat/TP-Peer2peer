...
void ClientThread(void *threadClientArgs)
{
	...
	case 1:
	{
		...
		printf("Demande d'enregistrement des fichiers reçue.\n");
		no_of_files = MsgReceiveNum(clntSock);
		printf("No. des fichiers devant être enregistré= %d\n",no_of_files);
		for(i=0;i<no_of_files;i++)
		{
		register_break = MsgReceiveNum(clntSock);
		if(register_break == 0)
		{
			printf("Enregistrement des fichiers abandonné\n");
			printf("No. des fichiers enregistres = %d\n",no_of_reg_files);
			break;
		}
	...
	case 2:
	{
		...
		printf("Demande de liste de fichiers reçue.\n");
	...
	case 3:
	{
		...
		printf("demande de localisation de fichier recue\n");
		MsgSendNum(clntSock,no_of_reg_files,sizeof(int));
		file_id=MsgReceiveNum(clntSock);
		/* Error handling */
		if(file_id > no_of_reg_files || file_id < 1)
			break;
		printf("demande des details sur le fichier : %d\n",file_id);
		count = 0;
		for(i=0;i<no_of_reg_files;i++)
		{
